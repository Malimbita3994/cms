...
@php
    ...
    $groupLabelKey = $subNavigation ? "sub_navigation_{$label}" : $label;
    $groupItemsId = filled($label) ? 'sidebar-group-' . \Illuminate\Support\Str::slug($label) : null;
@endphp

<li
    x-data="{ label: @js($groupLabelKey) }"
    data-group-label="{{ $groupLabelKey }}"
...
>
    @if ($label)
        @if ($collapsible)
            <button
                ...
                @if ($groupItemsId) aria-controls="{{ $groupItemsId }}" @endif
            ...
            >
                ...
                <span class="fi-sidebar-group-label">{{ $label }}</span>

                <span class="fi-sidebar-group-collapse-btn" aria-hidden="true">
                    {{ \Filament\Support\generate_icon_html(\Filament\Support\Icons\Heroicon::ChevronUp, size: \Filament\Support\Enums\IconSize::Small) }}
                </span>
            </button>
        @else
            <div
                @if ($sidebarCollapsible)
                    ...
                    x-transition:enter-end="fi-transition-enter-end"
                @endif
                class="fi-sidebar-group-btn fi-sidebar-group-btn--static"
            >
                @if ($icon)
                    {{ \Filament\Support\generate_icon_html($icon, size: \Filament\Support\Enums\IconSize::Large) }}
                @endif
    ...
    <ul
        @if ($groupItemsId) id="{{ $groupItemsId }}" @endif
        @if (filled($label))
            @if ($sidebarCollapsible)
